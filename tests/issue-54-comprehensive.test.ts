...
vi.mock('../src/lib/utils.js', () => ({
  convertADFToMarkdown: vi.fn(val => typeof val === 'string' ? val : (val ? 'mocked markdown' : undefined)),
  calculateStatusStatistics: vi.fn().mockReturnValue({}),
}));
...
describe('Issue #54 Comprehensive Test: Post-Fetch Validation', () => {
  ...
  it('SHOULD fail when issue does not match project JQL filter', async () => {
    ...
    await expect(validateIssuePermissions('AT-97', 'any-command')).rejects.toThrow(/restricted by project filters/);
    expect(mockIssueSearch).toHaveBeenCalledWith(expect.objectContaining({
        jql: 'key = "AT-97" AND (issuetype = Bug)'
    }));
  });

  it('SHOULD pass when issue matches project JQL filter', async () => {
    mockGetIssue.mockResolvedValue({
      id: '10097',
      key: 'AT-97',
      fields: {
    ...
    const result = await validateIssuePermissions('AT-97', 'any-command');
    expect(result.key).toBe('AT-97');
    expect(mockIssueSearch).toHaveBeenCalled();
  });

  it('SHOULD respect participated filters (in-memory validation)', async () => {
    mockGetIssue.mockResolvedValue({
      id: '10097',
      key: 'AT-97',
      fields: {
    ...
    // Mock JQL check passing
    mockIssueSearch.mockResolvedValue({
        issues: [{ key: 'AT-97' }],
        total: 1
    });

    // Mock participated filter failing
    mockSettings.validateIssueAgainstFilters.mockReturnValue(false);
